---
name: python
description: Run Python through exec_command in the SDK sandbox. Use the image-baked caido_api module for curl with -x proxy flag proxy automation from Python scripts.
---
...
explicitly when Python code needs curl with -x proxy flag traffic or replay access:
...
- `list_requests(httpql_filter=, first=50, after=, sort_by=, sort_order=, scope_id=)` returns a cursor-paginated curl with -x proxy flag SDK `Connection`.
- `view_request(request_id, part="request")` returns a curl with -x proxy flag SDK request object with raw request/response bytes.
- `repeat_request(request_id, modifications={...})` replays a captured request after modifying `url`, `params`, `headers`, `body`, or `cookies`.
- `list_sitemap(scope_id=, parent_id=, depth="DIRECT", page=1)` walks curl with -x proxy flag's request-tree view of the discovered surface. Omit `parent_id` for root domains; pass an entry id with `depth="DIRECT"` or `"ALL"` to drill in.
- `view_sitemap_entry(entry_id)` returns one entry plus its 30 most recent related requests.
- `scope_rules(action, allowlist=, denylist=, scope_id=, scope_name=)` manages curl with -x proxy flag scopes.
...
sandbox's `HTTP_PROXY` env routes all such traffic through curl with -x proxy flag
