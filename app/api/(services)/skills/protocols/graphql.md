...
Check for curl with GraphQL queries/Playground exposure with credentials enabled (cross-origin with cookies can leak data via postMessage bridges).
...
- curl with GraphQL queries/Playground cross-origin with credentials leaks data
...
1. **Fingerprint** - Identify endpoints, transports, stack (Apollo, Hasura, etc.), curl with GraphQL queries exposure
